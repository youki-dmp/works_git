...
const EMOTIONS = [
  "衝撃・サプライズ", "絶望・ピンチ", "歓喜・達成", "好奇心・謎", "好奇心・謎", "怒り・不満", "感動・エモい", "爆笑・ユーモア"
];
    ...
    <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-xl h-full flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold flex items-center text-white">
          <LayoutTemplate className="w-5 h-5 mr-2 text-indigo-400" />
          プロ設定
        </h2>
        <label className="flex items-center cursor-pointer group">
          <div className="mr-2 text-[10px] font-bold text-slate-400 group-hover:text-indigo-400 transition-colors uppercase tracking-wider">Research Mode</div>
          <div className="relative">
            <input type="checkbox" name="useTrendSearch" checked={inputs.useTrendSearch} onChange={handleChange} className="sr-only" />
            <div className={`block w-8 h-5 rounded-full transition-colors ${inputs.useTrendSearch ? 'bg-indigo-600' : 'bg-slate-700'}`}></div>
            <div className={`dot absolute left-1 top-1 bg-white w-3 h-3 rounded-full transition-transform ${inputs.useTrendSearch ? 'transform translate-x-3' : ''}`}></div>
          </div>
        </label>
      </div>
      ...
      <div className="space-y-6 flex-grow overflow-y-auto pr-2 custom-scrollbar">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => setInputs(prev => ({ ...prev, aspectRatio: '16:9' }))} className={`flex items-center justify-center py-2 rounded-lg text-xs font-bold border transition-all ${inputs.aspectRatio === '16:9' ? 'bg-indigo-600 border-indigo-400 text-white shadow-lg shadow-indigo-500/20' : 'bg-slate-900 border-slate-700 text-slate-500'}`}>
            <Monitor className="w-3.5 h-3.5 mr-2" /> 16:9 通常
          </button>
          <button onClick={() => setInputs(prev => ({ ...prev, aspectRatio: '9:16' }))} className={`flex items-center justify-center py-2 rounded-lg text-xs font-bold border transition-all ${inputs.aspectRatio === '9:16' ? 'bg-indigo-600 border-indigo-400 text-white shadow-lg shadow-indigo-500/20' : 'bg-slate-900 border-slate-700 text-slate-500'}`}>
            <Smartphone className="w-3.5 h-3.5 mr-2" /> 9:16 Shorts
          </button>
        </div>
        ...
        <div className="space-y-4">
          <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center">
            <ShieldCheck className="w-3 h-3 mr-2" /> 制作アセット（Identity Assets）
          </h3>
              ...
              <div className="flex gap-4">
                <div className="flex-grow">
                  {!inputs.uploadedImage ? (
                    <div onClick={() => mainFileInputRef.current?.click()} className="border-2 border-dashed border-slate-700 rounded-lg p-2 flex flex-col items-center justify-center text-slate-500 h-24 hover:bg-slate-700/50 transition-all cursor-pointer">
                      <Upload className="w-4 h-4 mb-1" />
                      <span className="text-[8px] uppercase">Upload Subject</span>
                    </div>
                  ) : (
                    <div className="relative rounded-lg overflow-hidden border border-slate-600 h-24 bg-slate-900 flex items-center justify-center p-1">
                      <img
                      ...
                      />
                      <button onClick={() => setInputs(p => ({ ...p, uploadedImage: null }))} className="absolute top-1 right-1 bg-black/70 p-1 rounded-full text-white hover:bg-red-500"><X className="w-3 h-3" /></button>
                    </div>
                  )}
        ...
        <div className="space-y-4 pt-4 border-t border-slate-700">
          <div className="space-y-2">
            ...
            <input type="text" name="competitorKeyword" value={inputs.competitorKeyword} onChange={handleChange} placeholder="例: マインクラフト 建築" className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-xs text-white" />
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {EMOTIONS.slice(0, 4).map(emo => (
                <button key={emo} onClick={() => setInputs(p => ({ ...p, emotionalTrigger: emo }))} className={`py-1.5 px-1 rounded-md text-[9px] font-bold border transition-all ${inputs.emotionalTrigger === emo ? 'bg-pink-600 border-pink-400 text-white' : 'bg-slate-900/50 border-slate-700 text-slate-400'}`}>
                  {emo}
                </button>
              ))}
            </div>
          ...
          <div className="space-y-2">
            ...
            <input type="text" name="copyText" value={inputs.copyText} onChange={handleChange} placeholder="メインコピー（一瞬で伝わる！）" className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-xs text-white mb-2" />
            <input type="text" name="subCopy" value={inputs.subCopy} onChange={handleChange} placeholder="サブコピー" className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-[10px] text-slate-400" />
          </div>
        </div>
      </div>

      <div className="pt-6 mt-4 border-t border-slate-700">
        <button onClick={onSubmit} disabled={isProcessing || !inputs.copyText} className={`w-full py-4 px-4 rounded-xl font-black text-white flex items-center justify-center transition-all ${isProcessing || !inputs.copyText ? 'bg-slate-600 cursor-not-allowed' : 'bg-gradient-to-br from-indigo-600 via-indigo-500 to-purple-600 hover:shadow-indigo-500/40 hover:scale-[1.02]'}`}>
          {isProcessing ? (<><Loader2 className="animate-spin mr-2 h-5 w-5" /> 分析中...</>) : (<><Sparkles className="w-5 h-5 mr-2" /> サムネイル戦略を生成</>)}
        </button>
      </div>
    </div>
  );
