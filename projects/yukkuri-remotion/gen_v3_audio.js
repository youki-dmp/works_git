...
async function main() {
    const dataPath = path.join(__dirname, 'tohoku_chubu_v3.json');
    const series = JSON.parse(fs.readFileSync(dataPath, 'utf8'));
    const speakers = { zundamon: 3, metan: 2, both: 2 };
    
    for (const id in series) {
        console.log(`--- Processing V3: ${id} ---`);
        const dialogue = series[id];
        const audioDir = path.join(__dirname, `video/public/audio/v3/${id}`);
    ...
    console.log('Tohoku and Chubu V3 audio generation complete!');
}
